...
clock = Label(f"Current Time: {current_time}")
        ...
        [
            ...
            VSplit(
                [
                    ...
                    Box(body=Frame(clock), padding=1, style="class:right-pane"),
                ]
            ),
        ]
    ),
...
# Digital Clock Task
def digital_clock_task(buffer):
    while True:
        current_time = time.strftime("%H:%M:%S")
        buffer._set_text(f"Digital Clock: {currtent_time}")
        time.sleep(1)

# Countdown Timer Task
def countdown_timer_task(buffer, seconds):
    while seconds > 0:
        buffer._set_text(f"Countdown Timer: {seconds} seconds remaining")
        time.sleep(1)
        seconds -= 1
    buffer._set_text("Countdown Timer: Done!")
...
def test_main():
    ...
    def _(event):
        ...
        seconds = int(prompt("Enter countdown time in seconds: "))
        event.app._current_task = countdown_timer_task

    @kb.add('d')
    def _(event):
        "Switch to digital clock."
        event.app._current_task = digital_clock_task

    # Start the application
    app.key_bindings = kb
    app._current_task = digital_clock_task

    # Run the application
    try:
        app.run()
        while True:
            task = app._current_task
            if task == digital_clock_task:
                digital_clock_task(buffer)
            elif task == countdown_timer_task:
                seconds = int(prompt("Enter countdown time in seconds: "))
                countdown_timer_task(buffer, seconds)
